import numpy as np
from MegaCon.variables import variables
from MegaCon.parameters import parameters  # Assuming parameters are stored in a similar way as variables
...
def constraints(x):
    ...
    Ceq[1] = -u_H/Y_H * S_S / (K_S + S_S) * (S_O / (K_OH + S_O) + eta_g * K_OH / (K_OH + S_O) * S_NO / (K_NO + S_NO)) * X_BH + \
             k_h * X_BH / (K_X * X_BH + X_S) * (S_O / (K_OH + S_O) + eta_h * K_OH / (K_OH + S_O) * S_NO / (K_NO + S_NO)) * X_S + \
             Q / V_a * (S_Sent - S_S)
    Ceq[2] = (1 - f_p) * b_H * X_BH + (1 - f_p) * b_A * X_BA - k_h * X_BH / (K_X * X_BH + X_S) * (S_O / (K_OH + S_O) + \
             eta_h * K_OH / (K_OH + S_O) * S_NO / (K_NO + S_NO)) * X_S + Q / V_a * (X_Sent - X_S)
    Ceq[3] = u_H * S_S / (K_S + S_S) * (S_O / (K_OH + S_O) + eta_g * K_OH / (K_OH + S_O) * S_NO / (K_NO + S_NO)) * X_BH - \
             b_H * X_BH + Q / V_a * (X_BHent - X_BH)
    Ceq[4] = u_A * S_NH / (K_NH + S_NH) * S_O / (K_OA + S_O) * X_BA - b_A * X_BA + Q / V_a * (X_BAent - X_BA)
    Ceq[5] = f_p * b_H * X_BH + f_p * b_A * X_BA + Q / V_a * (X_Pent - X_P)
    Ceq[6] = -(1 - Y_H) / (2.86 * Y_H) * u_H * S_S / (K_S + S_S) * K_OH / (K_OH + S_O) * S_NO / (K_NO + S_NO) * eta_g * \
             X_BH + u_A / Y_A * S_NH / (K_NH + S_NH) * S_O / (K_OA + S_O) * X_BA + Q / V_a * (S_NOent - S_NO)
    Ceq[7] = -u_H * S_S / (K_S + S_S) * (S_O / (K_OH + S_O) + eta_g * K_OH / (K_OH + S_O) * S_NO / (K_NO + S_NO)) * \
             i_XB * X_BH - u_A * (i_XB + 1 / Y_A) * S_NH / (K_NH + S_NH) * S_O / (K_OA + S_O) * X_BA + k_a * S_ND * X_BH + \
             Q / V_a * (S_NHent - S_NH)
    Ceq[8] = -k_a * X_BH * S_ND + k_h * X_BH / (K_X * X_BH + X_S) * (S_O / (K_OH + S_O) + eta_h * K_OH / (K_OH + S_O) * \
             S_NO / (K_NO + S_NO)) * X_ND + Q / V_a * (S_NDent - S_ND)
    Ceq[9] = b_H * (i_XB - f_p * i_XP) * X_BH + b_A * (i_XB - f_p * i_XP) * X_BA - k_h * X_BH / (K_X * X_BH + X_S) * \
             (S_O / (K_OH + S_O) + eta_h * K_OH / (K_OH + S_O) * S_NO / (K_NO + S_NO)) * X_ND + Q / V_a * (X_NDent - X_ND)
    Ceq[10] = -i_XB / 14 * u_H * (S_S / (K_S + S_S)) * (S_O / (K_OH + S_O)) * X_BH - ((1 - Y_H) / (14 * 2.86 * Y_H) + \
              i_XB / 14) * u_H * (S_S / (K_S + S_S)) * (K_OH / (K_OH + S_O)) * (S_NO / (K_NO + S_NO)) * eta_g * X_BH - \
              (i_XB / 14 + 1 / (7 * Y_A)) * u_A * S_NH / (K_NH + S_NH) * S_O / (K_OA + S_O) * X_BA + 1 / 14 * k_a * S_ND * \
              X_BH + Q / V_a * (S_alkent - S_alk)

    # Oxygen balance
    Ceq[11] = KLa - alfa * G_s * fracO2 * eta * 1333.3 / (V_a * SOST) * 1.024 ** (T - 20)
    Ceq[12] = Q * S_Oent - Q * S_O + KLa * (SOST - S_O) * V_a + ((-(1 - Y_H) / Y_H) * u_H * (S_S / (K_S + S_S)) * \
              (S_O / (K_OH + S_O)) * X_BH - (4.57 - Y_A) / Y_A * u_A * (S_NH / (K_NH + S_NH)) * (S_O / (K_OA + S_O)) * X_BA) * V_a
