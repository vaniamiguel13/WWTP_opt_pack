...
def InitPopulation(Problem: Dict[str, Any], InitialPopulation: List[Dict[str, Any]], Size: int, Conflag: int, CTol: float, CeqTol: float, NormType: Union[int, float], NormCons: int, *args: Any) -> Tuple[Dict[str, Any], Dict[str, np.ndarray]]:
    ...
    if InitialPopulation and not isinstance(InitialPopulation, list):
        ...
        # First evaluate one individual to determine the shape of constraint arrays
        temp_ind = np.array([np.random.uniform(low, high) for low, high in
                             zip(Problem['LB'][:Problem['Variables']], Problem['UB'][:Problem['Variables']])])
        _, temp_c, temp_ceq = ConEval(Problem, temp_ind, *args)

        # Initialize population structure with determined shapes
        Population = {
            'x': np.zeros((Size, Problem['Variables'])),
            'f': np.zeros((Size, Problem['Objectives'])),
            'c': np.zeros((Size, len(temp_c))),  # Adjust shape based on constraints
            'ceq': np.zeros((Size, len(temp_ceq))),  # Adjust shape based on constraints
        ...
        for i, ind in enumerate(InitialPopulation):
            Population['x'][i, :] = Bounds(ind['x'], Problem['LB'][:Problem['Variables']],
        ...
        for i in range(len(InitialPopulation), Size):
            Population['x'][i, :] = np.array([np.random.uniform(low, high) for low, high in
                                              zip(Problem['LB'][:Problem['Variables']],
                                                  Problem['UB'][:Problem['Variables']])])
            Problem, Population['f'][i, :] = ObjEval(Problem, Population['x'][i, :], *args)
            if Conflag:
                Problem, Population['c'][i, :], Population['ceq'][i, :] = ConEval(Problem, Population['x'][i, :], *args)
        ...
        if Conflag:
            for i in range(Size):
                if NormCons:
                    maxc = np.maximum(1, np.max(Population['c'], axis=0))
                    maxceq = np.maximum(1, np.max(np.abs(Population['ceq']), axis=0))
                    Population['Feasible'][i] = (
                                np.linalg.norm(np.maximum(0, Population['c'][i, :]) / maxc, ord=NormType) <= CTol and
                                np.linalg.norm(np.abs(Population['ceq'][i, :]) / maxceq, ord=NormType) <= CeqTol)
                else:
                    Population['Feasible'][i] = (
                                np.linalg.norm(np.maximum(0, Population['c'][i, :]), ord=NormType) <= CTol and
